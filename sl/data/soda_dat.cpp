...
namespace sunlight
{
    // client 0x561310, general purpose decompress function
    // [algorithm, library]
    // -> 1. UPX -> ucl v1.01 (https://github.com/korczis/ucl.git - v1.03)
    // -> 2. BZ2 -> libbzip2 (https://github.com/LuaDist/bzip2.git)
    // but soda.dat is only compressed by BZ2, so this function is partially implemented.
    ...
    auto SodaDat::CreateFrom(const std::filesystem::path& sodaDatFilePath) -> SodaDat
    {
        ...
        // client has logic for soda.unk2, soda.unk3
        // but a actual soda.dat file's unk2, unk3 is '0'
        // so it's implementation is dropped.
