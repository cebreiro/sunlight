...
namespace sunlight
{
    ...
    void Server::StartUp(uint16_t listenPort)
    {
        ...
        {
            SUNLIGHT_LOG_CRITICAL(_locator, fmt::format("[{}] fail to start. exception: {}",
                GetName(), e.what()));
        }
    ...
    void Server::Shutdown()
    {
        ...
        if (ec)
        {
            SUNLIGHT_LOG_CRITICAL(_locator, fmt::format("[{}] has a error on shutdown process. errorCode: [{}, {}]",
    ...
    void Server::OnAccept(const boost::system::error_code& ec, boost::asio::ip::tcp::socket socket)
    {
        if (ec)
        {
            ...
            {
                SUNLIGHT_LOG_CRITICAL(_locator,
    ...
    void Server::HandleAcceptError(const boost::system::error_code& ec)
    {
        if (ec)
        {
            SUNLIGHT_LOG_CRITICAL(_locator, fmt::format("[{}] fail to accept session. errorCode: [{}, {}]",
                GetName(), ec.value(), ec.message()));
        }
    }
