...
{
    Merchant::Merchant(StreamReader<std::vector<char>::const_iterator>& reader)
    {
    ...
    void MerchantTable::LoadFromFile(const std::filesystem::path& path)
    {
        SoxFile fileData = SoxFile::CreateFrom(path / "MERCHANT.sox");
    ...
    auto MerchantTable::Find(int32_t index) const -> const Merchant*
    {
    ...
    auto MerchantTable::Get() const -> const std::vector<Merchant>&
    {
        return _vector;
    }
}
