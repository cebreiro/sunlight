...
CREATE TABLE `characters_item` (
  ...
  `item_id` int(11) NOT NULL,
  `quantity` int(11) NOT NULL,
  `pos_type` enum('inventory','equipment','quick_slot','pick','vendor') NOT NULL,
  `page` tinyint(4) NOT NULL,
  `x` tinyint(4) NOT NULL,
  `y` tinyint(4) NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE `characters_skill` (
  ...
  KEY `characters_skill_cid_fk_idx` (`cid`),
...
BEGIN
	...
	IF ROW_COUNT() = 0 THEN
		SIGNAL SQLSTATE '45000' 
		SET MESSAGE_TEXT = 'fail to find target';
	END IF;
...
BEGIN
	...
	SELECT id, aid, name, hair, hair_color, face, skin_color, arms, running, gold,inventory_page,zone, stage, x, y, yaw
    ...
    SELECT id, job, level, cooldown, page, x, y 
...
BEGIN
	UPDATE characters
    SET zone = zone, stage = stage, x = x, y = y, yaw = yaw, arms = arms, running = running
...
BEGIN
	...
	IF ROW_COUNT() = 0 THEN
		SIGNAL SQLSTATE '45000' 
		SET MESSAGE_TEXT = 'fail to find target character';
	END IF;
...
BEGIN
    ...
    INSERT INTO temp
		( id )
	SELECT id FROM characters AS c WHERE c.aid = account_id AND c.deleted = 0 AND c.sid = sid;
...
-- Dump completed on 2024-10-05  9:55:36
